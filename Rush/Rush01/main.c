...
void	lines_to_array(int check_array[4][4], int permutations[24][4], int row_indices[4]);
...
*/
void	loop(int row_indices[4], int check_array[4][4], int permutations[24][4], int values[16])
{
	while (row_indices[0]++, row_indices[0] < 24)
	{
		...
		while (row_indices[1]++, row_indices[1] < 24)
		{
			...
			while (row_indices[2]++, row_indices[2] < 24)
			{
				...
				{
					lines_to_array(check_array, permutations, row_indices);
...
/*
1. Creates and initializes the variables needed for the loop function and calls it
...
/*
1. Checks if the character of argv[1] at index cur is in its place and if so adds it to the values array
...
/*
Main: retrieves the input string and converts it to an array of values then calls the resolve function with this array
*/
